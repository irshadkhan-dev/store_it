...
async function serverFnFetcher(url, args, handler) {
  ...
  if (reactRouter.isPlainObject(_first) && _first.method) {
    ...
    if (first.method === "GET") {
      const encodedPayload = reactRouter.encode({
        payload: serializer.startSerializer.stringify({
          data: first.data,
    ...
    if ((_a = response2.headers.get("content-type")) == null ? void 0 : _a.includes("application/json")) {
      const json = serializer.startSerializer.decode(await response2.json());
  ...
  if (contentType && contentType.includes("application/json")) {
    return serializer.startSerializer.decode(await response.json());
  } else {
...
function getFetcherRequestOptions(opts) {
  if (opts.method === "POST") {
    if (opts.data instanceof FormData) {
      opts.data.set("__TSR_CONTEXT", serializer.startSerializer.stringify(opts.context));
      return {
        body: opts.data
      };
    }
    return {
      body: serializer.startSerializer.stringify({
        data: opts.data ?? null,
...
async function handleResponseErrors(response) {
  if (!response.ok) {
    ...
    if (isJson) {
      throw serializer.startSerializer.decode(await response.json());
    }
...
//# sourceMappingURL=serverFnFetcher.cjs.map
