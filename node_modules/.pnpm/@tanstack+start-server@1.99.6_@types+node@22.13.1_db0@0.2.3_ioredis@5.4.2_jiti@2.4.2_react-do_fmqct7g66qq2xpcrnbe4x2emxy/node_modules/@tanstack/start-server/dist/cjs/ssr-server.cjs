...
function attachRouterServerSsrUtils(router, manifest) {
  router.ssr = {
    manifest,
    serializer: startClient.startSerializer
  };
  router.serverSsr = {
    injectedHtml: [],
...
function extractAsyncLoaderData(loaderData, ctx) {
  ...
  const replaced = replaceBy(loaderData, (value, path) => {
    if (value instanceof ReadableStream) {
      ...
      const deferredPromise = reactRouter.defer(value);
      const entry = {
        type: "promise",
        path,
        id: extracted.length,
  ...
  const initCode = `__TSR_SSR__.initMatch(${jsesc(
    {
      ...
      extracted: extracted == null ? void 0 : extracted.map((entry) => reactRouter.pick(entry, ["type", "path"])),
      ...
      return `__TSR_SSR__.resolvePromise(${jsesc(
        {
          matchId: match.id,
          id: entry.id,
          promiseState: entry.promise[reactRouter.TSR_DEFERRED_PROMISE]
        },
        {
          isScriptContext: true,
          wrap: true,
